...
if (empty(htmlspecialchars(trim($_GET['date_fact'])))) {
	...
	$date_fact = htmlspecialchars(trim($_GET['date_fact']));
}
...
$Vente = new Vente();
	...
	if ($Lot_prod->date_lot > $date_fact) {
		$errors[]= 'Date lot est supérieur  a date facture !';
	}
	...
	if (empty($errors)){
if ($Vente->existe()) {
	$existe_prod= Vente::trouve_vente_vide_par_id_prod($Vente->id_lot,$Vente->id_person);
...
<?php
			...
			echo '  ","Attention !");
				...
				if (!empty($table_vantes)) {
					$last_client = Vente::trouve_last_client_par_id_admin($Vente->id_person,$Vente->id_societe);
