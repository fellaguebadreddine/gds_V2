...
if ( (isset($_GET['id'])) && (is_numeric($_GET['id']))  ) { 
     $id = intval($_GET['id']);
     $Fact = Facture_avoir_vente::trouve_par_id($id);
...
if (isset($_GET['action']) &&( $_GET['action'] =='vente'  )) {
			...
			$(document).ready(function(){
				 setTimeout(function() {
           toastr.success(" Facture Vente N°'.sprintf("%04d", $Fact->N_facture).'/'.$date['year'].' Ajouter avec succès","Très bien !");
        },300);
        });
			</script>';
					...
					<div class="col-xs-12">
						<table class="table  table-bordered " >
							...
							<tbody>
								...
								$list_vantes = Avoir_vente::trouve_vente_par_facture($Fact->id_facture);
								}
