...
if ($user->type == "administrateur"){
	...
	if (empty($errors)){
		...
		$facturesImportationAvant = facture_importation ::trouve_importation_par_societe_etat_g50_not_selected($nav_societe->id_societe,$dateMonthAvant,$dateMonthAvantFin);
...
// g50 form
	...
	if (empty($errors)){
		 ...
		 if (isset($_GET['id_facture']))  {
			...
			$total_somme_vente= 0;
			$som_vente_esono =0;
			$som_vente_esono_esp=0;
			$som_vente_esono_non_esp=0;
			$total_vente_non_espece_exonore =0;
			$total_vente_espece_exonore =0;
			foreach ($idArray1 as $idArrays){
				if (!empty($idArrays)){				
					...
					$exon_taps_espece=	vente::trouve_vente_par_facture_exono_espece($idArrays);
					$exon_taps_non_espece=	vente::trouve_vente_par_facture_exono_non_espece($idArrays);					
					...
					foreach ($exon_taps_espece as $exon_tap){ 						
						$som_vente_esono_esp += $exon_tap->Ht ;						
					}
					foreach ($exon_taps_non_espece as $exon_tap_no_esp){ 						
						$som_vente_esono_non_esp += $exon_tap_no_esp->Ht ;						
					}
			...
			$som_vente_esono = $som_vente_esono_esp + $som_vente_esono_non_esp;
			$total_vente_espece_exonore = $total_venteeES - $som_vente_esono_esp;
			$total_vente_non_espece_exonore = $total_vente - $som_vente_esono_non_esp;
			$total_somme_vente = $total_vente_espece_exonore + $total_vente_non_espece_exonore + $som_vente_esono ;
