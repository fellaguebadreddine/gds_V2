...
class Update_reglement_client {
	
	protected static $nom_table="update_reglement_client";
	protected static $champs = array('id','id_client', 'id_societe','id_solde','mode_paiment','id_facture','type_fact','date', 'somme','timbre','etat','update_reglement','id_person');
	public $id;
	public $id_client;
	public $id_societe;
	public $id_solde;
	...
	public $timbre;
	public $etat;
	public $update_reglement;
	public $id_person;

	

 	public static function truncate_table($id) {
		return self::trouve_par_sql("delete from   ".self::$nom_table." WHERE etat  = 0 and update_reglement = 0 and id_facture = {$id}");
  }
  ...
  public static function trouve_Reglement_vide_par_admin($id) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where etat = 0 and id_facture =  {$id} and type_fact = 0  ");
  }
  public static function trouve_Reglement_vide_par_admin_avoir($id) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where etat = 0 and id_facture =  {$id} and type_fact = 1  ");
  }	

  public function delete_Reglement_vide_par_admin($id){
global $bd;
$sql = "DELETE FROM ".self::$nom_table;
$sql .= " where id_facture = {$id}" ;
$sql .=" and etat = 0 and type_fact = 0 ";
$bd->requete($sql);
return($bd->affected_rows() > 0) ? true : false ;
	}
  public function delete_Reglement_vide_par_admin_avoir($id){
global $bd;
$sql = "DELETE FROM ".self::$nom_table;
$sql .= " where id_facture = {$id}" ;
$sql .=" and etat = 0 and type_fact = 1 ";
$bd->requete($sql);
return($bd->affected_rows() > 0) ? true : false ;
	}
  public static function trouve_somme_Reglement_vide_par_admin($id) {
    $result_array = self::trouve_par_sql("SELECT SUM(somme) as somme  FROM ".self::$nom_table." where etat = 0 and id_facture  = {$id}  and type_fact = 0   LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function trouve_somme_Reglement_vide_par_admin_avoir($id) {
    $result_array = self::trouve_par_sql("SELECT SUM(somme) as somme  FROM ".self::$nom_table." where etat = 0 and id_facture  = {$id}  and type_fact = 1   LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function trouve_last_Reglement_vide_par_admin($id) {
    $result_array = self::trouve_par_sql("SELECT *  FROM ".self::$nom_table." where etat = 0 and type_fact = 0  and id_facture =  {$id}   ORDER BY `id`  DESC LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function trouve_last_Reglement_vide_par_admin_avoir($id) {
    $result_array = self::trouve_par_sql("SELECT *  FROM ".self::$nom_table." where etat = 0   and id_facture =  {$id} and type_fact = 1  ORDER BY `id`  DESC LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function trouve_somme_and_timbre_vide_par_admin($id) {
    $result_array = self::trouve_par_sql("SELECT SUM(somme+timbre) as somme ,SUM(timbre) as timbre FROM ".self::$nom_table." where etat = 0 and id_facture  = {$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE id_solde = '".$this->id_solde."' and id_facture = ".$this->id_facture." and id_person = ".$this->id_person."";
    $sql .= " LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}
...
public static function trouve_fact_num_clie($id=0) {
  ...
  public static function trouve_par_classee($classe=0){
    ...
    public static function trouve_reglement_vide_par_id_solde($id,$id_person,$id_facture) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE   id_facture = {$id_facture} and id_solde = {$id} and id_person = {$id_person} and type_fact = 0 LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function trouve_reglement_vide_par_id_solde_avoir($id,$id_person,$id_facture) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE   id_facture = {$id_facture} and id_solde = {$id} and id_person = {$id_person} and type_fact = 1 LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
