...
class Facture {
	...
	protected static $nom_table="facture";
	protected static $champs = array('num_fac', 'date_fac','time', 'somme', 'type_fac');
	public $num_fac;
	public $date_fac;
	public $time;
	public $somme;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE num_fac = '".$this->num_fac."' ";
  ...
  public static function trouve_par_id($id=0) {
   ...
   public static function trouve_par_last($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table."  ORDER BY `num_fac` DESC LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->num_fac)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE num_fac =". $bd->escape_value($this->num_fac) ;
$bd->requete($sql);
