...
class Unite {
	...
	protected static $nom_table="unite_mesure";
	protected static $champs = array('id_unite','id_societe','unite');
	public $id_unite;
	public $id_societe;
	public $unite;
	...
	public static function trouve_unite_par_societe($id){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_societe ={$id}";
    return  self::trouve_par_sql($q);
	}
	public static function trouve_unite_par_id_unite($id){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_unite ={$id}";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_unite={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_unite)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_unite =". $bd->escape_value($this->id_unite) ;
$bd->requete($sql);
