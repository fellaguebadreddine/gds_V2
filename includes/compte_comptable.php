...
class Compte_comptable {
  ...
  public static function trouve_par_id($id=0) {
	...
	public static function trouve_par_code($id=0,$id_societe) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code={$id} and id_societe={$id_societe} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
