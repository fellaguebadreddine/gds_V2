...
class Marque {
	...
	protected static $nom_table="marque";
	protected static $champs = array('id_marque','marque');
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_marque='$id' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_marque)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_marque =". $bd->escape_value($this->id_marque) ;
$bd->requete($sql);
