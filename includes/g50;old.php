...
if ($user->type == "administrateur"){
	...
	if (empty($errors)){
		...
		$facturesNonEspese  = Facture_vente::trouve_facture_par_societe_month($nav_societe->id_societe,$date_db,$date_fin);
...
if (isset($_POST['submit']) && $action == 'save_g50'){
	...
	$id_facture_importation = unserialize($_POST['id_facture_importation']);
	$month = htmlentities(trim($_POST['month']));
...
if ($g50->existe()) {
			...
			foreach ($id_facture_importation as $idArrayImportations){
				...
				$Upfact_imortation->modifier();
				
			}
			foreach ($id_facture_importation as $FactImportations){
				$Upfact_imortation = Achat_importation::trouve_par_id($FactImportations);
				
				$Upfact_imortations = Facture_importation::trouve_par_id($Upfact_imortation->id_facture);
				
				$Upfact_imortations->id_g50 =$last_G50->id;
				$Upfact_imortations->modifier();
...
<script>
	  ...
	  $(document).on('click','#valider_g50', function() {
		...
		var year_g50 = <? echo $thisyear ;?>;
			window.location = 'g50.php?action=save_g50&date_g50='+date_g50+'&year_g50='+year_g50+'&id_facture='+idVente+'&id_achat='+idAchat+'&id_importation='+idImportation;
