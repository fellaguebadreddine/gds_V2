								...
								<div class="input-group">
									...
									<option>Selectionner dans la liste</option>
											...
											<?php  foreach ($banques as $banque) {
												?>
												<option value="<?php if(isset($banque->id_banque)){$bank = Banque::trouve_par_id($banque->id_banque);
										if (isset($bank->id_banque)) {
											echo  'b_'.$bank->id_banque;
...
$(document).on('change','#id_nature', function() {
    var id = this.value;
   var id_societe = <?php if (isset($nav_societe->id_societe)) {echo $nav_societe->id_societe;}  ?>;
     $('.id_tier').load('ajax/tiers_comptes.php?id='+id+'&id_societe='+id_societe,function(){       
    });
