...
class Releve_comptes {
	
	protected static $nom_table="releve_comptes";
	protected static $champs = array('id','id_societe','id_banque','id_caisse','ref_releve','libelle','somme_debit','somme_credit','date','date_valid','id_nature','id_tier','details','mode_paiment','facture_scan','random','id_user');
	public $id;
	public $id_societe;
	public $ref_releve;
	public $libelle;
	...
	public static function trouve_releve_par_societe_and_Exercice($id,$date_debut,$date_fin){
    ...
    return  self::trouve_par_sql($q);
		...
		public static function trouve_releve_par_societe($id){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_societe ={$id}";
    return  self::trouve_par_sql($q);
	}
	public static function trouve_releve_par_id_banque($id,$id_societe){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_banque ={$id}";
	$q .= " AND id_societe ={$id_societe}";
    return  self::trouve_par_sql($q);
	}
	public static function trouve_releve_par_id_caiss($id,$id_societe){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_caisse ={$id}";
	$q .= " AND id_societe ={$id_societe}";
    return  self::trouve_par_sql($q);
	}
    ...
    $sql .= "WHERE ref_releve = '".$this->ref_releve."'";
    $sql .= "LIMIT 1";
  ...
  public static function trouve_par_random($id=0) {
    ...
    public static function trouve_par_operation_and_type($id,$type) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_op_auto ={$id} and type_op = {$type} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }  
