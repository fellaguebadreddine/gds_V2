...
class Compte {
	...
	public static function trouve_compte_banque_defrent_id_banque($id_societe,$id_banque){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_societe = $id_societe";
	$q .= " AND id_banque != $id_banque";
    return  self::trouve_par_sql($q);
	}


	public static function annuler_par_defaut($id){
  ...
  public static function trouve_par_id_banque_and_id_societe($id=0,$id_societe) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_banque ={$id} AND id_societe ={$id_societe} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function trouve_compte_par_defaut_and_id_societe($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_societe ={$id} AND compte_defaut=1 LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
