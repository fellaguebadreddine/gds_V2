...
class Caisse {
	...
	protected static $nom_table="caisse";
	protected static $champs = array('id_caisse','id_societe','Code','solde', 'Designation','comptes_caisse','journal','auxiliere_achat');
	public $id_caisse;
	public $id_societe;
	public $Code;
	public $solde;
	public $Designation;	
	public $comptes_caisse;
	public $journal;
  ...
  public function nom_compler() {
	...
	public static function trouve_caisse_defrent_id_caisse($id_societe,$id_caisse){
		$q =  "SELECT * FROM ".self::$nom_table;
		$q .= " WHERE id_societe = $id_societe";
		$q .= " AND id_caisse != $id_caisse";
		return  self::trouve_par_sql($q);
		}
    ...
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_caisse={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
	}	
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_caisse)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_caisse =". $bd->escape_value($this->id_caisse) ;
$bd->requete($sql);
