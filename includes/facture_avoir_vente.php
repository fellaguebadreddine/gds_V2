...
class Facture_avoir_vente {
	
	protected static $nom_table="facture_avoir_vente";
	protected static $champs = array('id_facture','N_facture','Num_facture','id_client','id_societe', 'date_fac','date_valid','time', 'somme_ht','somme_Ht_achat','somme_tva','timbre','Remise','somme_ttc', 'mode_paiment', 'etat','Montant_paiement','date_reglement','facture_scan','id_g50','random');
	public $id_facture;
	public $N_facture;
	public $Num_facture;
	public $id_client;
	...
	public static function trouve_avoir_vente_par_id_fact($id){
		$q =  "SELECT * FROM ".self::$nom_table;
		$q .= " WHERE id_facture ={$id}";
		$q .= " AND id_g50 = 0 ";
		return  self::trouve_par_sql($q);
		}
	public static function trouve_avoir_par_societe_and_Exercice_and_id_client($id,$id_client,$date_debut,$date_fin){
		$q =  "SELECT * FROM ".self::$nom_table;
	...
	public static function trouve_avoir_achat_par_societe_g50($id,$d1,$d2){
		$q =  "SELECT * FROM ".self::$nom_table;
		$q .= " WHERE id_societe ={$id}";
		$q .= " AND id_g50 = 0 ";
	...
	public static function trouve_avoir_achat_par_societe_not_selected_g50($id,$d1,$d2){
		$q =  "SELECT * FROM ".self::$nom_table;
		$q .= " WHERE id_societe ={$id}";
		$q .= " AND id_g50 = 0 ";
		$q .= " AND date_fac BETWEEN  '{$d1}' and '{$d2}'";
