...
class Achat {
	...
	protected static $nom_table="achat";
	protected static $champs = array('id', 'id_facture','Reference_fact','date_fact','id_societe', 'Quantite', 'Prix', 'id_fournisseur', 'id_prod','id_famille','Code','Designation', 'id_person','Unite','Empl','Remise','Ht','id_tva','Ttva','Tva','total','somme_ht','somme_tva','somme_ttc');
	public $id;
	public $id_facture;
  ...
  public static function trouve_tva_immobil_par_achat($id){
	$q =  " SELECT achat.Tva FROM facture_achat LEFT JOIN achat ON facture_achat.id_facture = achat.id_facture JOIN produit ON achat.id_prod = produit.id_pro";
	$q .= " WHERE achat.id_facture ={$id}";
	$q .= " AND produit.immobil =1";
	return  self::trouve_par_sql($q);
}
  ...
  public static function trouve_par_id_prod($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_prod ={$id} ORDER BY `id` DESC LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
