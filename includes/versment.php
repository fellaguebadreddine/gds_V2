...
class Versment {  
	...
	protected static $nom_table="versment_cl";
	protected static $champs = array('id_ver','montant','total', 'num_clie', 'date_vers');
  ...
  public static function trouve_fact_vide() {
    ...
    public static function trouve_client($id=0) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where num_clie = {$id} ");
  }
  
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_ver={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function trouve_par_client($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE num_clie ={$id} ");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_ver)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_ver =". $bd->escape_value($this->id_ver) ;
$bd->requete($sql);
