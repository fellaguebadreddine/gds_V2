...
class Facture_achat {
	...
	public static function trouve_facture_non_valide_par_id_fournisseur($id){
		$q =  "SELECT facture_achat.*, mode_paiement_societe.mode_paiement FROM reglement_fournisseur INNER JOIN mode_paiement_societe ON reglement_fournisseur.mode_paiment = mode_paiement_societe.id INNER JOIN facture_achat ON facture_achat.id_facture = reglement_fournisseur.id_facture";
		$q .= " WHERE reglement_fournisseur.id_fournisseur ={$id}";
		$q .= " AND mode_paiement_societe.mode_paiement=3";
		$q .= " AND reglement_fournisseur.type_fact =1";
		return  self::trouve_par_sql($q);
		}
