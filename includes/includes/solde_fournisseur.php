...
class Solde_fournisseur {
	
	protected static $nom_table="solde_fournisseur";
	protected static $champs = array('id','id_fournisseur', 'id_societe','debit','solde', 'credit', 'date','somme', 'reference', 'mode_paiment', 'banque', 'caisse','id_person','random','etat');
	public $id;
	...
	public static function trouve_par_mode_and_fournisseur($id_societe,$id_four,$mode){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_societe ='{$id_societe}'";
	$q .= " and id_fournisseur ={$id_four}";
	$q .= " and mode_paiment ={$mode}";
	$q .= " and etat = 0";
    return  self::trouve_par_sql($q);
	}	
			public static function trouve_par_mode_and_fournisseur_negatif($id_societe,$id_fournisseur,$mode){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_societe ='{$id_societe}'";
	...
	public static function trouve_par_all_mode_and_fournisseur($id_societe,$id_four,$mode){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_societe ='{$id_societe}'";
	$q .= " and id_fournisseur ={$id_four}";
	$q .= " and mode_paiment ={$mode}";
...
public static function trouve_solde_par_id_fournisseur($id){
	$q =  "SELECT * FROM ".self::$nom_table;
  ...
  public static function trouve_versement_par_id_fournisseur($id) {
    $result_array = self::trouve_par_sql("SELECT IFNULL( SUM(debit),0) as somme , IFNULL( sum(solde),0) as solde, IFNULL( sum(debit-solde),0) as debit FROM ".self::$nom_table." WHERE id_fournisseur ={$id} ");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
