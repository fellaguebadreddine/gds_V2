...
class Achat_importation {
	
	protected static $nom_table="achat_importation";
...
public static function trouve_importation_par_facture($id){
    ...
    $sql .= "WHERE id_prod = '".$this->id_prod."' and id_facture = '0' and id_person = '".$this->id_person."' ";
    $sql .= " and is_frais_annexes = 1 LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
	 ...
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE id_prod = '".$this->id_prod."' and id_facture = '0' and id_person = '".$this->id_person."' ";
    $sql .= "  and is_frais_annexes = 0 LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}  
  ...
  public static function somme_tva_frais_par_facture($id,$id_societe) {
    $result_array = self::trouve_par_sql("SELECT SUM(Tva) as somme_tva FROM ".self::$nom_table." where id_societe = {$id_societe}   and id_facture = {$id}  and is_frais_annexes = 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function somme_timbre_frais_par_facture($id,$id_societe) {
    $result_array = self::trouve_par_sql("SELECT SUM(timbre) as timbre FROM ".self::$nom_table." where id_societe = {$id_societe}   and id_facture = {$id}  and is_frais_annexes = 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function somme_ttc_frais_par_facture($id,$id_societe) {
    $result_array = self::trouve_par_sql("SELECT SUM(total + timbre) as somme_ttc FROM ".self::$nom_table." where id_societe = {$id_societe}   and id_facture = {$id}  and is_frais_annexes = 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function somme_VA_frais_par_facture($id,$id_societe) {
    $result_array = self::trouve_par_sql("SELECT SUM(valeur_DA) as valeur_DA FROM ".self::$nom_table." where id_societe = {$id_societe}   and id_facture = {$id}  and is_frais_annexes = 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function somme_ht_par_facture($id) {
    $result_array = self::trouve_par_sql("SELECT id, id_fournisseur, SUM(Ht) as somme_ht FROM ".self::$nom_table." where id_facture = {$id}");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_achat_vide_par_id_prod($id,$id_person) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE   id_facture = 0 and id_prod = {$id} AND id_person = {$id_person} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
