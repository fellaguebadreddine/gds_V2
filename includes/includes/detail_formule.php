...
class Detail_formule {
	
	protected static $nom_table="detail_formule";    
	protected static $champs = array('id','id_formule','id_Matiere_Premiere','id_lot','qte');
	public $id;
	public $id_formule;
...
return($bd->affected_rows() > 0) ? true : false ;
	...
	public static function trouve_detail_formule_par_societe($id){
	$q = "SELECT * FROM `formule`,detail_formule WHERE formule.id = detail_formule.id_formule and `id_societe` = {$id}";
	...
	public static function trouve_formule_par_id_Matiere_Premiere($id){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_Matiere_Premiere ={$id}";
	 return  self::trouve_par_sql($q);
	}	
