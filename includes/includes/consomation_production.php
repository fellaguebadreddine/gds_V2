...
class Consomation_production {
	
	protected static $nom_table="consomation_production";    
	protected static $champs = array('id','id_production','id_societe','id_formule','id_Matiere_Premiere','id_lot','id_famille','qte','Ht_production','date');
	public $id;
	public $id_production;
	public $id_societe;
	public $id_formule;
	public $id_Matiere_Premiere ;
	public $id_lot;
	public $id_famille;
	public $qte;
	public $Ht_production;
	public $date;
	

  public function delete_Detail_formule_par_id_formule($id){
global $bd;
$sql = "DELETE FROM ".self::$nom_table;
$sql .= " where id_formule =  {$id} " ;
$bd->requete($sql);
return($bd->affected_rows() > 0) ? true : false ;
	...
	public static function trouve_detail_par_id_formule($id){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_formule ={$id}";
	 return  self::trouve_par_sql($q);
	}		

	public static function trouve_detail_par_id_production($id){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_production ={$id}";
	 return  self::trouve_par_sql($q);
	}		
    ...
    $sql .= "WHERE id_formile = '".$this->id_formile."' ";
   ...
   public static function trouve_par_id_production($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_production  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
