...
class Facture_avoir_achat {
	
	protected static $nom_table="facture_avoir_achat";
	...
	public static function trouve_avoir_par_societe_and_Exercice_and_id_fournisseur($id,$id_fournisseur,$date_debut,$date_fin){
	...
	public static function trouve_avoir_achat_par_id_fact($id){
		$q =  "SELECT * FROM ".self::$nom_table;
  ...
  public static function sum_tva_avoir_achat($id=0) {
    $result_array = self::trouve_par_sql("SELECT SUM (somme_tva) AS somme_tva FROM ".self::$nom_table." WHERE id_facture IN ('{$id}') ");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
